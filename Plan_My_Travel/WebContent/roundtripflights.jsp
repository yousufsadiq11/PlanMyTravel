...
<%
		ArrayList<flight_details> roundtrip=(ArrayList<flight_details>)request.getAttribute("roundtrip");

		
		for(flight_details f2:roundtrip){%>
		
			<h3><b>Source: <%=f2.getSource() %> </b></h1>
			<h3><b>Destination: <%=f2.getDestination() %> </b></h1>
		
	<%break;} %>
	<% 
		for(flight_details f3:roundtrip){
		

		 %>
		
<button class="accordion" name="x" value="<%=f3.getFlight_number() %>"><%=f3.getFlight_name() %></button>
<div class="panel">
 	<p>Flight Number: <%= f3.getFlight_number()%>	Arrival: <%= f3.getArrival() %>	Departure: <%= f3.getDeparture() %> </p>
  <p>Economy Seats: <%=f3.getEconomy_seat_count() %>	Business Seats: <%=f3.getBusiness_seat_count() %></p>
  <form method="post" action=book.jsp>
 <p><input type="radio" name="test" value="<%=f3.getFlight_id() %>" >Book</p>
<input type="submit">
</form>
</div>
